...
public class RandomForestBinClassifier extends Classifier {
    ...
    public int Classify(FlowData f){
        ...
        ArrayList<Long> aux=f.ToArrayList();
        //aux.add(f.dscp);
        aux.add(f.firstTime);
        aux.add(f.flast);
        aux.add(f.blast);
        //aux.forEach(number->log.info(Long.toString((number))));
        if(rfc.predict(aux)>0){
            log.warn("Found class "+Integer.toString(rfc.predict(aux)));
        }else{
            log.info("Normal Flow");
        }
        //log.error("Found class"+Integer.toString(rfc.predict(aux)));
